...
<svelte:head>
	<title>Zakat Kebun/Ikan - ZakatFlow</title>
</svelte:head>
...
<div class="max-w-2xl mx-auto" transition:fade={{ duration: 400 }}>
	...
	<div class="mb-8" transition:fly={{ y: -20, duration: 600 }}>
		<h1 class="text-4xl font-display font-bold text-white mb-2">🌴 Zakat Hasil Kebun/Ikan</h1>
		<p class="text-white/60 text-lg">Hitung zakat dari hasil kebun dan perikanan</p>
	</div>
	...
	<div transition:fly={{ y: 20, delay: 100, duration: 500 }}>
		<Card class="p-8">
			<form class="space-y-6">
				<div>
					<Input label="Pendapatan Bersih (Rp)" type="number" placeholder="Pendapatan bersih dari hasil kebun/ikan" min="0" />
				</div>

				<div>
					<Input label="Harga Emas per Gram (Rp)" type="number" placeholder="Harga emas saat ini" />
				</div>

				<div class="bg-white/5 rounded-xl p-4 border border-white/10">
					<h3 class="text-white font-semibold mb-2">Informasi Zakat Hasil Kebun/Ikan</h3>
				...
				<div class="bg-primary-500/20 border border-primary-500/30 rounded-xl p-4">
					<p class="text-white/60 text-sm mb-1">Total Zakat</p>
					<p class="text-3xl font-bold text-primary-400">Rp 0</p>
					<p class="text-white/40 text-xs mt-1">Pendapatan bersih: Rp 0 | Nisab: Rp 0</p>
				</div>
