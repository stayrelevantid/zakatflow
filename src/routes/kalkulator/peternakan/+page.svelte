<script lang="ts">
	...
	function calculateZakatHewan(jenis: JenisHewan, jumlah: number): { jumlahHewan: number; keterangan: string } {
		if (jenis === 'sapi') {
			if (jumlah < 30) return { jumlahHewan: 0, keterangan: `Jumlah ${jumlah} ekor belum mencapai nisab minimal 30 ekor` };
			if (jumlah >= 30 && jumlah <= 39) return { jumlahHewan: 1, keterangan: '1 ekor tabi\' (anak sapi umur 1 tahun)' };
			if (jumlah >= 40 && jumlah <= 59) return { jumlahHewan: 1, keterangan: '1 ekor musinn (sapi umur 2 tahun)' };
			if (jumlah >= 60 && jumlah <= 69) return { jumlahHewan: 2, keterangan: '2 ekor tabi\'' };
			if (jumlah >= 70 && jumlah <= 79) return { jumlahHewan: 2, keterangan: '1 ekor musinn + 1 ekor tabi\'' };
			...
			if (jumlah >= 100 && jumlah <= 109) return { jumlahHewan: 3, keterangan: '1 ekor musinn + 2 ekor tabi\'' };
			// For 110+ : continues pattern
			const tabi = Math.floor(jumlah / 30);
			return { jumlahHewan: tabi, keterangan: `${tabi} ekor tabi' (perhitungan detail sesuai tabel nisab)` };
		}
		
		if (jenis === 'kambing') {
			if (jumlah < 40) return { jumlahHewan: 0, keterangan: `Jumlah ${jumlah} ekor belum mencapai nisab minimal 40 ekor` };
		...
		if (jenis === 'unta') {
			if (jumlah < 5) return { jumlahHewan: 0, keterangan: `Jumlah ${jumlah} ekor belum mencapai nisab minimal 5 ekor` };
			if (jumlah >= 5 && jumlah <= 9) return { jumlahHewan: 0, keterangan: '1 ekor kambing (untuk 5-9 unta)' };
			if (jumlah >= 10 && jumlah <= 14) return { jumlahHewan: 0, keterangan: '2 ekor kambing (untuk 10-14 unta)' };
			if (jumlah >= 15 && jumlah <= 19) return { jumlahHewan: 0, keterangan: '3 ekor kambing (untuk 15-19 unta)' };
			if (jumlah >= 20 && jumlah <= 24) return { jumlahHewan: 0, keterangan: '4 ekor kambing (untuk 20-24 unta)' };
			if (jumlah >= 25 && jumlah <= 35) return { jumlahHewan: 1, keterangan: '1 ekor bintu makhad (unta umur 1 tahun)' };
			if (jumlah >= 36 && jumlah <= 45) return { jumlahHewan: 1, keterangan: '1 ekor bintu labun (unta umur 2 tahun)' };
			if (jumlah >= 46 && jumlah <= 60) return { jumlahHewan: 1, keterangan: '1 ekor hiqqah (unta umur 3 tahun)' };
			if (jumlah >= 61 && jumlah <= 75) return { jumlahHewan: 1, keterangan: '1 ekor jadza\' (unta umur 4 tahun)' };
			...
			return { jumlahHewan: Math.ceil(jumlah / 50), keterangan: `Perhitungan khusus untuk ${jumlah} ekor unta` };
		}
...
<div class="max-w-2xl mx-auto" transition:fade={{ duration: 400 }}>
	...
	<div class="mb-8" transition:fly={{ y: -20, duration: 600 }}>
		...
		<p class="text-white/40 text-sm mt-2">Zakat dapat dibayar dengan ternak atau diwangkan (uang)</p>
	</div>
	...
	<div transition:fly={{ y: 20, delay: 100, duration: 500 }}>
		<Card class="p-8">
			<form class="space-y-6">
				...
				{:else}
					<div>
						<Input label="Nilai per Hewan (Rp) - Opsional" type="number" placeholder="Untuk estimasi nilai" bind:value={nilaiPerHewan} min="0" />
						<p class="text-white/40 text-xs mt-1">Opsional: untuk informasi estimasi nilai dalam rupiah</p>
					</div>
				{/if}
					...
					<div class="bg-primary-500/20 border border-primary-500/30 rounded-xl p-4">
						...
						{#if result.wajibZakat}
							...
							{#if metodeZakat === 'ternak'}
								<p class="text-3xl font-bold text-primary-400">{result.zakatHewan} ekor {jenisHewan === 'sapi' ? 'sapi/kerbau' : jenisHewan === 'kambing' ? 'kambing' : 'unta'}</p>
								{#if nilaiPerHewan > 0}
									<p class="text-white/60 text-sm mt-1">Estimasi nilai: {formatCurrency(result.zakatRupiah)}</p>
								{/if}
							{:else}
								<p class="text-3xl font-bold text-primary-400">{formatCurrency(result.zakatRupiah)}</p>
								<p class="text-white/60 text-sm mt-1">Setara dengan {result.zakatHewan} ekor {jenisHewan === 'sapi' ? 'sapi/kerbau' : jenisHewan === 'kambing' ? 'kambing' : 'unta'}</p>
							{/if}
						{:else}
				...
				{:else if result}
					<div class="flex gap-4">
						<Button type="button" onclick={resetForm} variant="outline" class="flex-1">Hitung Ulang</Button>
					</div>
				{/if}
			</form>
		</Card>
