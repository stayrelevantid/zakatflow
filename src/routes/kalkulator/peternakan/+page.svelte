	...
	async function handleSave() {
		...
		try {
			await createTransaksi({
				kategori: 'Zakat Peternakan/Perikanan',
				nilaiHarta: pendapatanBersih,
				zakatWajib: result.zakatWajib,
				metode: '2.5% dari pendapatan bersih',
				status: 'Belum Bayar',
...
<svelte:head>
	<title>Zakat Peternakan/Perikanan - ZakatFlow</title>
</svelte:head>
...
<div class="max-w-2xl mx-auto" transition:fade={{ duration: 400 }}>
	...
	<div class="mb-8" transition:fly={{ y: -20, duration: 600 }}>
		<h1 class="text-4xl font-display font-bold text-white mb-2">🐄 Zakat Peternakan/Perikanan</h1>
		<p class="text-white/60 text-lg">Hitung zakat dari hasil peternakan dan perikanan</p>
	</div>
	...
	<div transition:fly={{ y: 20, delay: 100, duration: 500 }}>
		<Card class="p-8">
			<form class="space-y-6">
				<div>
					<Input label="Pendapatan Bersih (Rp)" type="number" placeholder="Pendapatan bersih dari hasil peternakan/perikanan" bind:value={pendapatanBersih} min="0" />
				</div>
				...
				<div class="bg-white/5 rounded-xl p-4 border border-white/10">
					<h3 class="text-white font-semibold mb-2">Informasi Zakat Peternakan/Perikanan</h3>
